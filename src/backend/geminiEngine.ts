...
export class GeminiEngine {
   ...
   */
  public async analyzeSnapshot(snapshot: UnifiedSnapshot): Promise<GeminiAIAnalysis> {
    const apiKey = process.env.GEMINI_API_KEY;
      ...
      try {
        ...
        if (response.text) {
          ...
          return {
            ...
            persianAnalysis: parsed.persianAnalysis || 'تحلیل جمینی بر اساس همگرایی تکنیکال و فیلترهای ریسک صادر شد.',
            englishAnalysis: parsed.englishAnalysis || 'Gemini analysis generated based on technical alignment and risk filters.',
   ...
   */
  private buildAnalysisPrompt(snapshot: UnifiedSnapshot): string {
...
Analyze the following live unified market snapshot and return a strictly valid JSON response.
...
Return JSON matching this exact structure:
{
  ...
  "persianAnalysis": "Short technical justification in Persian language (2-3 sentences)",
